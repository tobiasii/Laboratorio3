...
wait	LDR R2, [R0] ; leitura do estado atual
	...
	LDR R1, [R0, #GPIO_PORT_DIR_R ] ; leitura do estado anterior
	ORR R1, R2 ; bit de saída
	STR R1, [R0, #GPIO_PORT_DIR_R ] ; escrita do novo estado

	LDR R0, =GPIO_PORTN_BASE
	LDR R1, [R0, #GPIO_PORT_DEN_R ] ; leitura do estado anterior
	ORR R1, R2 ; habilita função digital
	STR R1, [R0, #GPIO_PORT_DEN_R ] ; escrita do novo estado
